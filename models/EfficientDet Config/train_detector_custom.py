...
from mAP_evaluation_custom import evaluate_coco
...
class Detector():
	...
	def Train_Dataset(self, root_dir, coco_dir, img_dir, set_dir, batch_size=8, image_size=512, use_gpu=True, num_workers=3):
		...
		self.system_dict["local"]["training_set"] = CocoDataset(root_dir=self.system_dict["dataset"]["train"]["root_dir"] + "/" + self.system_dict["dataset"]["train"]["coco_dir"],
															...
															transform = transforms.Compose([Normalizer(), Resizer(common_size = self.system_dict["params"]["image_size"])]) #Augmenter(),
															)
	...
	def Train(self, num_epochs=2, model_output_dir="trained/"):
		...
		#modified.
		file_path1="/content/train_loss.txt"
		with open(file_path1,"w") as info_file1:
		  print("Epoch","Classification_Loss","Regression_Loss","Total_Loss",file=info_file1)
		
		#modified.
		file_path2="/content/valid_loss.txt"
		with open(file_path2,"w") as info_file2:
		  print("Epoch","Classification_Loss","Regression_Loss","Total_Loss",file=info_file2)
		...
		if(self.system_dict["dataset"]["val"]["status"]):
			...
			for epoch in range(self.system_dict["params"]["num_epochs"]):
				...
				writer.add_scalar('Train/Regression_loss', reg_loss, epoch)
				writer.add_scalar('Train/Classfication_loss (focal loss)', cls_loss, epoch)

				#Modified.
				with open (os.path.abspath(file_path1),"a") as another1:
				  print(epoch+1,float(cls_loss),float(reg_loss),total_loss,file=another1)     
				...
				if epoch % self.system_dict["params"]["val_interval"] == 0:
					...
					#Modified.          
					print("Reached")
					with open (os.path.abspath(file_path2),"a") as another2:
					  print(epoch+1,float(cls_loss),float(reg_loss),np.mean(loss),file=another2) 
