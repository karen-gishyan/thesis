...
class Detector():
    ...
    def Train(self, num_epochs=2, output_model_name="final_model.pt"):
        ...
        with open(file_path,"w") as info_file:
          print("Epoch","Iteration","Classification_Loss","Regression_Loss","Running_Loss",file=info_file)
                ...
                try:
                    ...
                    if(iter_num % self.system_dict["params"]["print_interval"] == 0):
                        ...
                        with open (os.path.abspath(file_path),"a") as another:
	                        print(epoch_num,iter_num,float(classification_loss),float(regression_loss),np.mean(self.system_dict["local"]["loss_hist"]),file=another)
